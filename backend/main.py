...
app.mount("/", StaticFiles(directory=PUBLIC_DIR, html=True), name="public")UPLOAD_DIR = os.path.join(PUBLIC_DIR, "uploads")
os.makedirs(UPLOAD_DIR, exist_ok=True)
...
def login_company(auth: CompanyAuth, api_key: str = Depends(get_api_key)):
    try:
        with engine.connect() as connection:
            ...
            if result:
                company_id = result[0]
                return {"message": f"Welcome back, {auth.company_name}!", "company_id": company_id}
            else:
                raise HTTPException(status_code=401, detail="Invalid Company Name or PIN!")
    except HTTPException:
        raise
    ...
    try:
        with engine.connect() as connection:
            result = connection.execute(
                ...
                {"new_pin": data.new_pin, "company_id": data.company_id, "current_pin": data.current_pin            ).fetchone()
            if result:
                connection.commit()
                return {"message": "PIN updated securely in the cloud!"}
            else:
